...
int main(int argc, char *argv[])
{
	...
	while( (argz<argc) && (argv[argz][0] == '-') ) {
		...
		} else if (strcmp(argv[argz], "-thispart") == 0) {
                        sscanf(argv[++argz], "%d", &nThisPart);
			nThisPart--; //use 0 based index
		} else if (strcmp(argv[argz], "-totalpart") == 0) {
                        sscanf(argv[++argz], "%d", &nTotalPart);
		} else {
	...
	if (print_help) {	// print error/usage message ?
		...
		fprintf(stderr, "       -totalpart <f>:\t for parallelization, divide LD computation into this many parts\n");
		fprintf(stderr, "       -thispart <d>:\tfor parallelization, compute this part of the whole problem\n");
		exit(1);
	}	
	...
	if ( nTotalPart == 1) {
		...
		string sNewOutFileName(out_file_name);
		sNewOutFileName = sNewOutFileName + "_" + std::to_string(nThisPart + 1) + "_of_" + std::to_string( nTotalPart) + ".txt";
		outstream = fopen( sNewOutFileName.c_str(), "w");
	}
	...
	while ( getline(inputFile, line) ) {
		...
		if (nJobCount % nTotalPart != nThisPart) {
			continue;
		}
