...
$bArbitraryPutMajorRead = false; // arbitrarily put the allele with more reads, when this == true, $nAlternativeReadRatio is set to 0.5
...
while(count($argv) > 0) {
	...
	case '-r':
	    ...
	    break;
	case '-M':
	    $bArbitraryPutMajorRead = (trim(array_shift($argv))=="yes");
	    break;
	case '-l':
...
if ($bArbitraryPutMajorRead) {
	$nAlternativeReadRatio = 0.5;
}
...
//echo all the parameters
	    ...
	    echo("\tRatio of reads supporting the alternative allele (-r), \n\tfor example, 1 means 0 reads supporting ref,\n\t 1 supporting alt, or 0 to 2, 0 to 3 etc.\n\t if doesn't pass this, masked as N:".fnB2S($nAlternativeReadRatio).PHP_EOL);
            echo("\tDo not mask as N, but arbitrarily assign the allele with more reads (-r is set to 0.5).\n\tIn case of tights, the allele is assigned in the following order: A>C>G>T:  ".fnB2S($bArbitraryPutMajorRead).PHP_EOL);
...
function fnProcessContig($sContigName, &$sRefSeq) {
	...
	global $nMaxGapSize, $sRefGenome, $sRefTaxon, $sOutFolder, $arrBCFs,$arrCoverages, $bPenalizeCoverage, $arrBCFUtils, $nSparseCutoff, $nMinDepth, $nMinQual , $nShortestAln, $bIncludeRefNPos, $bCodeIndelOrMultiNonRefAlleleAsN, $bTreatNAsRefBase , $nAlternativeReadRatio, $bArbitraryPutMajorRead;
	...
	for ($i=0;$i< $nLenContig;$i++) {
			...
			foreach($arrBCFUtils as $sTaxon => $oBCFTool) {
				...
				if (substr($arrPos1[0],0,1) !== $sRefBase ) {
					//check if refbase is ambiguous and bcf ref base is ambiguous
					if ($arrPos1[0] == 'N' && strpos( 'MRWSYKVHDBN',$sRefBase)!==false ) { // if the fasta file uses IUPAC ambiguity code, and the bcf is "N", it's not inconsistent.
				...
				if ($sAltBase != $sRefBase) {
					...
					if (count($arrDepthForAlleles) ==2) {
						$bAltIsMajor = (($arrDepthForAlleles[1] / ($arrDepthForAlleles[0] + $arrDepthForAlleles[1] ))>=$nAlternativeReadRatio);
						$bAlleleTight = ($arrDepthForAlleles[0] == $arrDepthForAlleles[1] );
					} 
					else {
						$bAltIsMajor = false;
					}
				...
				if ($bArbitraryPutMajorRead) {
					if ($bAlleleTight) { // in case allele count is tight
						$bAltIsMajor = ($sAltBase < $sRefBase); // if the ASCII code of the alternative allele is lower (alphabetically earlier), then arbitrarily treat as major
					}

					$sBaseToPut = ($bAltIsMajor)? $sAltBase : $sRefBase;
					$arrCurrBase[$sTaxon] =  (($arrPos1[2] >= $nMinQual && $arrPos1["DP"]>= $nRequiredMinDepth)? $sBaseToPut : 'N');

				} else {
					$arrCurrBase[$sTaxon] =  (($arrPos1[2] >= $nMinQual && $arrPos1["DP"]>= $nRequiredMinDepth && $bAltIsMajor)? $sAltBase : 'N');
				}
				//print_r($arrPos1);
