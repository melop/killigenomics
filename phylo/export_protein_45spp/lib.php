...
class FastaHack{ //fastahack wrapper
	...
	public function ForceCreateIndex() {
		if ( file_exists($this->sFastaDb . '.fai')) {
			unlink($this->sFastaDb . '.fai');
		}
		echo("Creating fastahack index $this->sFastaDb ...\n");
		$sCMD = FASTAHACK_PATH." -i \"$this->sFastaDb\"";
		exec( $sCMD, $arrRet, $nReturnVal);
	}
	...
	public function GetCDSRegion($sChr, $nStart, $nEnd) {
		...
		if ($sChr != $this->sBufferScfld || $nStart < $this->nBufferStart || $nEnd > $this->nBufferEnd) { 
			...
			if (!$this->bContigOK) {
				return false; //something wrong with contig, don't do it.
			}
			// update buffer
			$this->sBufferScfld = $sChr;
			$this->nBufferStart = $nStart;
			$this->nBufferEnd = $nStart + intval(FASTAHACK_BUFFERSIZE) - 1;
			$this->nBufferEnd = ($this->nBufferEnd <= $nEnd)? $nEnd+1 : $this->nBufferEnd;
			$arrRet = array();
			$nReturnVal = -1;
			$sCMD = FASTAHACK_PATH." \"$this->sFastaDb\" -r ".escapeshellcmd($this->sBufferScfld).':'.$this->nBufferStart.'..'.$this->nBufferEnd;
			//echo($sCMD);
			//die();
			if ($sChr == "GapFilledScaffold_625") {
				echo($sCMD.PHP_EOL);
			}
			exec( $sCMD, $arrRet, $nReturnVal);
			//$sLn = shell_exec(FASTAHACK_PATH." $this->sFastaDb -r ".$this->sBufferScfld.':'.$this->nBufferStart.'..'.$this->nBufferEnd);
			if ($nReturnVal != 0 ) {
				echo("Warning: $sChr: $nStart .. $nEnd not found in $this->sFastaDb .\n");
...
class RAXML {
	...
	public function GetBestTree($bKeepTmpFolder = true) {
		...
		$sCmd = RAXML_PATH . " --no-bfgs -T 4 -f a -s \"". RAXML::IN_FILE_NAME . "\" -p 23333 -m GTRGAMMA -x 23 -N 100 -n ".RAXML::RAXML_OUT;
		exec( $sCmd );
		...
		if (!file_exists($sRaxmlLnOut )) {
			echo("RAxML did not generate output $sRaxmlLnOut! ");
			chdir(dirname(__FILE__) );
			return array("Success"=>false, "Tree"=>"" , "tmpfolder" => $sTempDir);
		}
		else {
		...
		return array("Success"=>true, "Tree"=>$sTreeRet , "tmpfolder" => $sTempDir);
