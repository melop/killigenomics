...
$sOutputPrefix = "ret";
$nMissingCutoff = 0.9; //at most allow 50% missing for the gene per taxon
...
$h12codonOutTable = fopen($sOutputPrefix."_codon12_table.txt" , "w");
...
fwrite($h4folddegenOutTable, "GeneName\tScfld" 
."\t"."Pos"
...
function fnProcessAln($sGeneName,$arrFullCodingSeq) {
	global   $sGTFFile, $arrMissingRules, $oGTF, $h4folddegenOutTable, $h12codonOutTable;
		...
		if (!$arrAlnRet["stopcodon"] ) { //there is no stop codon, export SNP
			...
			for($nPos=0;($nPos+2)<$nSeqLen;$nPos+=3) {
				...
				foreach($arrFullCodingSeq  as $sTaxon => $sSeq) {
					...
					if (strpos($sCodon , 'N') !== false ) { //if anything is N, do not write this position.
						$bWriteThis = false;
						break;
					}
				...
				if ( $bWriteThis && (count( array_unique($arrFoundSNPs1)) <=2) && (count( array_unique($arrFoundSNPs2)) <=2) ) { //biallelic, or constant site output
				...
				foreach($arrFullCodingSeq  as $sTaxon => $sSeq) {
					...
					$arrTrans = fnDNA2Peptide($sCodon , false, false);
					if ($sAminoAcid=="") {

						$sAminoAcid = $arrTrans[0];
					}
					$bIs4Fold = fnIs4FoldDegen($sCodon);
					if ( $bIs4Fold ===-1 || $bIs4Fold===false ) {
						/*if ($nPos==99 && $sGeneName=="TTC39C(1of3)-mRNA-1") {
							die("not 4 fold");
						}*/
						continue 2;
					}

					if ( $arrTrans[0] != $sAminoAcid) { //some other snps changed because amino acid is different
						/*if ($nPos==99 && $sGeneName=="TTC39C(1of3)-mRNA-1") {
							die("not equal $sCodon and $sAminoAcid");
						}*/
						continue 2; //give up
					}

					//print($sCodon." ");
					$arrFoundSNPs[] = substr($sCodon, 2, 1); //put the 3rd position

				}

				//print_r($arrFoundSNPs);
				if ( count( array_unique($arrFoundSNPs)) <=2) { //biallelic, or constant site output

					fwrite($h4folddegenOutTable, "$sGeneName\t"
					.implode("\t", fnGenePosToGenomePos($sGeneName, $nPos+3))
					."\t".implode("\t" , $arrFoundSNPs)
					.PHP_EOL);
