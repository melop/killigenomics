...
$sGTFFile = "/beegfs/group_dv/data/human_GRCh37.p13/refseq.longestisoform.renamechr.gff3";
...
$sVariantSites = array("../polarized/$sPop.chr*.txt.gz" , "../polarized/der_".$sPop.".chr*.txt.gz");
...
foreach($sVariantSites as $sVarFile) {
	$hVarFile = popen("zcat -f $sVarFile" , 'r');
...
$arrGenes = $oGTF->GetGenes('BestRefSeq');
	...
	foreach($arrRNA['CDSs'] as $nExonStart => $nExonEnd ) { 
		...
		for($i=0;$i<$nInFrameBases;$i+=3) {
			...
			for ($nCodonOffset=0;$nCodonOffset < 3; $nCodonOffset++) {
				...
				$sAncCodon = $sDerCodon = $sCodon ;
				$sAncCodon[$nCodonOffset] = $arrVariantSites[$sScfld][$nPos][0];
				$sDerCodon[$nCodonOffset] = $arrVariantSites[$sScfld][$nPos][1];
				$sInfoStr = "\t$nCodonOffset\t$sCodon\t".implode("\t", $arrVariantSites[$sScfld][$nPos])."\t$sAncCodon\t$sDerCodon";
				if ($sClassified == 'syn') {
					fwrite($hSynOutTable, "$sScfld\t".( $nPos-1 ) . "\t".$nPos.$sInfoStr.PHP_EOL ); //bed format, start pos is 0 based, end pos is 1 based
				}

				if ($sClassified == 'non-syn') {
					fwrite($hNonSynOutTable, "$sScfld\t".( $nPos-1 ) . "\t".$nPos.$sInfoStr.PHP_EOL ); //bed format, start pos is 0 based, end pos is 1 based
				}
			}
