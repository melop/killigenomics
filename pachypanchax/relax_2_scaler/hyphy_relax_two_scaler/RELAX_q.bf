...
relax.json    = { terms.json.analysis: relax.analysis_description,
                  ...
                  terms.json.fits : {},
                  terms.json.timers : {},
                  terms.json.test_resultsqOne: {},
                  terms.json.test_results : {}
                  };

relax.relaxation_parameter        = "relax.K";
relax.relaxation_parameterQ        = "relax.Q";
...
terms.relax.q          = "adjustment parameter Q for positive sites";
terms.relax.q_range    = {
        terms.lower_bound: "0",
...
relax.display_orders = {terms.original_name: -1,
                        ...
                        relax.null_name: 4,
                        relax.qOne_name: 3,
                        relax.partitioned_descriptive_name: 6
                       };
...
if (relax.model_set == "Minimal") {
    ...
    ExecuteAFile (PATH_TO_CURRENT_BF + "constraint-definition-q/" + relax.module.params);
} else {
    ExecuteAFile (PATH_TO_CURRENT_BF + "constraint-definition-q/standard");
}
...
parameters.DeclareGlobalWithRanges (relax.relaxation_parameterQ, 1, 0, 10000);
...
if (relax.numbers_of_tested_groups == 2) {
	io.ReportProgressMessageMD ("RELAX", "alt", "Fitting the alternative model to test K != 1 Q != 1");
} else {
...
relax.model_to_relax_parameterQ = {};
...
if (relax.numbers_of_tested_groups == 2) {
	...
	relax.model_to_relax_parameterQ  ["relax.test"] = relax.relaxation_parameterQ;
	...
	utility.ForEachPair  (relax.branch_sets, "_key_", "_value_", "
		...
		relax.model_to_relax_parameterQ  [relax.k] = relax.relaxation_parameterQ + '_' + _value_;
...
for (relax.k = 0; relax.k < relax.numbers_of_tested_groups; relax.k += 1) {
    ...
    if (relax.k > 0) { // not the reference group    	 
    	 relax.declare_parameter_with_ranges (relax.k, relax.model_to_relax_parameter  [relax.model_nmsp], relax.model_to_relax_parameterQ  [relax.model_nmsp]);
    	 
    	 if (relax.k > 1) {
    	 	relax.relax_parameter_terms [relax.k] = terms.relax.k + " for " + relax.model_to_group_name[relax.model_nmsp];
    	 	relax.relax_parameterQ_terms [relax.k] = terms.relax.q + " for " + relax.model_to_group_name[relax.model_nmsp];
    	 	...
    	 	model.generic.AddGlobal (relax.model_object_map[relax.model_nmsp], relax.model_to_relax_parameterQ  [relax.model_nmsp], relax.relax_parameterQ_terms [relax.k]);
    	 } else {
    	 	...
    	 	model.generic.AddGlobal (relax.model_object_map[relax.model_nmsp], relax.model_to_relax_parameterQ  [relax.model_nmsp], terms.relax.q);
    	 }
		 ...
		 for (relax.i = 1; relax.i <= relax.rate_classes; relax.i += 1) {
           ...
           relax.impose_distributional_constraints_alternative (relax.i, 
				 model.generic.GetGlobalParameter (relax.model_object_map[relax.model_nmsp] , terms.AddCategory (terms.parameters.omega_ratio,relax.i)),
				  ...
				  relax.model_to_relax_parameterQ  [relax.model_nmsp]
				);
...
function relax.FitMainTestPair () {
	...
	if (relax.numbers_of_tested_groups == 2) {
		...
		relax.fitted.Q = estimators.GetGlobalMLE (relax.alternative_model.fit,terms.relax.q);

		relax.fitted.mainmodel.K = relax.fitted.K;
		relax.fitted.mainmodel.Q = relax.fitted.Q;

		io.ReportProgressMessageMD("RELAX", "alt", "* Relaxation/intensification parameter (K) = " + Format(relax.fitted.K,8,2));
		io.ReportProgressMessageMD("RELAX", "alt", "* Adjustment parameter for positively selected sites (Q) = " + Format(relax.fitted.Q,8,2));
        ...
        if (relax.skip_convergence_checking != TRUE) {
            ...
            if (Abs (relax.best_samples[1] - relax.best_samples[0]) < 5.) { // could be diagnostic of convergence problems
                ...
                if (relax.alternative_model.fit.take2 [terms.fit.log_likelihood] > relax.alternative_model.fit[terms.fit.log_likelihood]) {
                    ...
                    relax.fitted.Q = estimators.GetGlobalMLE (relax.alternative_model.fit.take2,terms.relax.q);
                    io.ReportProgressMessageMD("RELAX", "alt-2", "* Relaxation/intensification parameter (K) = " + Format(relax.fitted.K,8,2));
                    io.ReportProgressMessageMD("RELAX", "alt-2", "* adjustment parameter (Q) = " + Format(relax.fitted.Q,8,2));
	...
	// NULL Model with Q == 1, i.e. the positive sites do not change

	selection.io.startTimer (relax.json [terms.json.timers], "RELAX Q=0 model fitting", 4);

	io.ReportProgressMessageMD ("RELAX", "qOne", "Fitting the qOne (K != 0, Q := 1) model");

	for (relax.k = 1; relax.k < relax.numbers_of_tested_groups; relax.k += 1) {
		relax.model_nmsp = relax.model_namespaces[relax.k ];
		if (relax.k > 1) {
		    relax.impose_distributional_constraints_qOne (relax.k, model.generic.GetGlobalParameter (relax.model_object_map[relax.model_nmsp] , terms.AddCategory (terms.parameters.omega_ratio,relax.k)),
		                  ...
		                  model.generic.GetGlobalParameter (relax.model_object_map[relax.model_nmsp] , relax.relax_parameterQ_terms[relax.k])
				);
		} else {
		     relax.impose_distributional_constraints_qOne (relax.k, model.generic.GetGlobalParameter (relax.model_object_map[relax.model_nmsp] , terms.AddCategory (terms.parameters.omega_ratio,relax.k)),
		                  model.generic.GetGlobalParameter (relax.model_object_map[relax.reference_model_namespace] , terms.AddCategory (terms.parameters.omega_ratio,relax.k)),
		                  model.generic.GetGlobalParameter (relax.model_object_map[relax.model_nmsp], terms.relax.k),
		                  model.generic.GetGlobalParameter (relax.model_object_map[relax.model_nmsp], terms.relax.q)
				  );
	...
	relax.qOne_model.fit = estimators.FitExistingLF (relax.alternative_model.fit[terms.likelihood_function], relax.model_object_map);
	io.ReportProgressMessageMD ("RELAX", "qOne", "* " + selection.io.report_fit (relax.qOne_model.fit, 9, relax.codon_data_info[terms.data.sample_size]));
	relax.LRT = math.DoLRT (relax.qOne_model.fit[terms.fit.log_likelihood], relax.alternative_model.fit[terms.fit.log_likelihood],  relax.numbers_of_tested_groups-1);

	if (relax.numbers_of_tested_groups == 2) {
		relax.fitted.K = estimators.GetGlobalMLE (relax.qOne_model.fit,terms.relax.k);
		relax.fitted.Q = estimators.GetGlobalMLE (relax.qOne_model.fit,terms.relax.q);
		...
		relax.report_multi_class_rates (relax.qOne_model.fit, FALSE);
	}

	relax.json [terms.json.test_resultsqOne] = relax.LRT;
	(relax.json [terms.json.test_resultsqOne])[terms.relax.k] = relax.fitted.K;
	(relax.json [terms.json.test_resultsqOne])[terms.relax.q] = relax.fitted.Q;
	(relax.json [terms.json.test_resultsqOne])[terms.relax.k + "_Q_unconstraint"] = relax.fitted.mainmodel.K;
	(relax.json [terms.json.test_resultsqOne])[terms.relax.q + "_Q_unconstraint"] = relax.fitted.mainmodel.Q;


	selection.io.json_store_lf (relax.json,
								relax.qOne_name ,
								relax.qOne_model.fit[terms.fit.log_likelihood],
								relax.qOne_model.fit[terms.parameters] + 9 , // +9 comes from CF3x4
								relax.codon_data_info[terms.data.sample_size],
								relax.distribution_for_json,
								relax.display_orders[relax.qOne_name]
							);

	selection.io.json_store_branch_attribute(relax.json, relax.qOne_name, terms.branch_length, relax.display_orders[relax.qOne_name],
												 0,
												 selection.io.extract_branch_info((relax.qOne_model.fit[terms.branch_length])[0], "selection.io.branch.length"));


	console.log ("----\n## Test for Q := 1 [Positive selection]");
	...
	selection.io.stopTimer (relax.json [terms.json.timers], "RELAX Q=0 model fitting");
	...
	relax.LRT = math.DoLRT (relax.null_model.fit[terms.fit.log_likelihood], relax.qOne_model.fit[terms.fit.log_likelihood],  relax.numbers_of_tested_groups-1);
	...
	(relax.json [terms.json.test_results])[terms.relax.q] = relax.fitted.Q;
