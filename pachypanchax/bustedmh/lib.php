	...
	*/
	...
	for ($i=0;$i<=strlen($DNA)-3;$i+=3) {
		...
		if (!$bDiscardN && $bHasN) {
			...
			$AminoAcid = (array_key_exists($Codon, $CodonMap)? $CodonMap[$Codon]:"X");
		}
...
class HyPhyUtil {
	...
	do {
	  ...
	  chdir($this->sCurrDir);
          ...
          //Read the template control file: this part is no longer needed in the hyphy develop version.
	/*
        ...
        public function RunRELAX($bKeepTmpFolder=true, $sParameterization = "standard") {
		...
		$sNexusFile = $this->sTmpFileBase . HyPhyUtil::IN_FILE_NAME;
		$sNexusFileParam = $this->sTmpFileBase . HyPhyUtil::IN_FILE_NAME . ".$sParameterization.nex";
                fnWriteText( $sNexusFile , $sNexus);
				$this->arrRet["tmpfolder"] = $this->sTmpFileBase;
                $this->arrRet["Success"] = true;
                ...
                //Run Ctrl file 1: ========================================
                chdir($this->sTmpFileBase ); //go to temp directory
                echo("Running RELAX $sParameterization parameterization...\n");

		exec("ln -sf $sNexusFile $sNexusFileParam");
                $sCMD = HYPHY_PATH . " ".  HYPHY_RELAX_SCRIPT;//HyPhyUtil::PREFIX_CTRL_FILE1 . ".bf";
		$sCMD .= "  --alignment $sNexusFileParam --code Universal --reference R --test T --models Minimal --parameterization $sParameterization > $sNexusFileParam.log 2>&1 ";
                echo($sCMD . "\n");
                ...
                $sF1 = fnReadText(  $sNexusFileParam . ".RELAX.json"); //read output.
		...
		$arrJSONret = json_decode($sF1 , true);
                if (is_null($arrJSONret) ) {
					...
					$this->arrRet["MG94xREV_param"] = ""; //loglik of MG94XREV
					$this->arrRet["NULL_param"] = ""; //Loglik of null model assuming K=1 (no relax or intensification of selection)
					$this->arrRet["Alternative_param"] = ""; //loglik of alt model, k estimated.
					$this->arrRet["MG94xREV_AICc"] = ""; //loglik of MG94XREV
					$this->arrRet["NULL_AICc"] = ""; //Loglik of null model assuming K=1 (no relax or intensification of selection)
					$this->arrRet["Alternative_AICc"] = ""; //loglik of alt model, k estimated.
		...
		$this->arrRet["MG94xREV"] = $arrJSONret["fits"]["MG94xREV with separate rates for branch sets"]["Log Likelihood"]; //loglik of MG94XREV
		$this->arrRet["NULL"] = $arrJSONret["fits"]["RELAX null"]["Log Likelihood"]; //Loglik of null model assuming K=1 (no relax or intensification of selection)
		$this->arrRet["Alternative"] = $arrJSONret["fits"]["RELAX alternative"]["Log Likelihood"]; //loglik of alt model, k estimated.
		$this->arrRet["qOne"] = $arrJSONret["fits"]["RELAX qOne"]["Log Likelihood"]; //loglik of alt model, k estimated.

		$this->arrRet["K_q_free"] = $arrJSONret["0"]["relaxation or intensification parameter_Q_unconstraint"]; //estimated k
		$this->arrRet["Q"] = $arrJSONret["0"]["adjustment parameter Q for positive sites_Q_unconstraint"]; //estimated k
		$this->arrRet["K_q_1"] = $arrJSONret["0"]["relaxation or intensification parameter"]; //estimated k
		$this->arrRet["LRT_pos_sel"] = $arrJSONret["0"]["LRT"]; //likelihood ratio
		$this->arrRet["P_pos_sel"] = $arrJSONret["0"]["p-value"]; //p value with 1 degree of freedom. chi sq.
		$this->arrRet["LRT_relax"] = $arrJSONret["test results"]["LRT"]; //likelihood ratio
		$this->arrRet["P_relax"] = $arrJSONret["test results"]["p-value"]; //p value with 1 degree of freedom. chi sq.

		$this->arrRet["R_omega0"] = $arrJSONret["fits"]["RELAX alternative"]["Rate Distributions"]["Reference"][0]['omega']; //omega 0 (<1) of reference branches.
		$this->arrRet["R_omega0_prop"] = $arrJSONret["fits"]["RELAX alternative"]["Rate Distributions"]["Reference"][0]['proportion']; // proportion of omega 0 (<1) of reference branches.
		$this->arrRet["R_omega1"] = $arrJSONret["fits"]["RELAX alternative"]["Rate Distributions"]["Reference"][1]['omega']; //omega 1 (near 1) of reference branches.
		$this->arrRet["R_omega1_prop"] = $arrJSONret["fits"]["RELAX alternative"]["Rate Distributions"]["Reference"][1]['proportion']; // proportion of omega 1 (near 1) of reference branches.
		$this->arrRet["R_omega2"] = $arrJSONret["fits"]["RELAX alternative"]["Rate Distributions"]["Reference"][2]['omega']; //omega 2 (>1) of reference branches.
		$this->arrRet["R_omega2_prop"] = $arrJSONret["fits"]["RELAX alternative"]["Rate Distributions"]["Reference"][2]['proportion']; // proportion of omega 2 (>1) of reference branches.
		$this->arrRet["T_omega0"] = $arrJSONret["fits"]["RELAX alternative"]["Rate Distributions"]["Test"][0]['omega']; //omega 0 (<1) of Test branches.
		$this->arrRet["T_omega0_prop"] = $arrJSONret["fits"]["RELAX alternative"]["Rate Distributions"]["Test"][0]['proportion']; // proportion of omega 0 (<1) of Test branches.
		$this->arrRet["T_omega1"] = $arrJSONret["fits"]["RELAX alternative"]["Rate Distributions"]["Test"][1]['omega']; //omega 1 (near 1) of Test branches.
		$this->arrRet["T_omega1_prop"] = $arrJSONret["fits"]["RELAX alternative"]["Rate Distributions"]["Test"][1]['proportion']; // proportion of omega 1 (near 1) of Test branches.
		$this->arrRet["T_omega2"] = $arrJSONret["fits"]["RELAX alternative"]["Rate Distributions"]["Test"][2]['omega']; //omega 2 (>1) of Test branches.
		$this->arrRet["T_omega2_prop"] = $arrJSONret["fits"]["RELAX alternative"]["Rate Distributions"]["Test"][2]['proportion']; // proportion of omega 2 (>1) of Test branches.
                
		$this->arrRet["MG94xREV_param"] = $arrJSONret["fits"]["MG94xREV with separate rates for branch sets"]["estimated parameters"]; //loglik of MG94XREV
		$this->arrRet["NULL_param"] = $arrJSONret["fits"]["RELAX null"]["estimated parameters"]; //Loglik of null model assuming K=1 (no relax or intensification of selection)
		$this->arrRet["Alternative_param"] = $arrJSONret["fits"]["RELAX alternative"]["estimated parameters"]; //loglik of alt model, k estimated.
		$this->arrRet["qOne_param"] = $arrJSONret["fits"]["RELAX qOne"]["estimated parameters"]; //loglik of alt model, k estimated.
		$this->arrRet["MG94xREV_AICc"] = $arrJSONret["fits"]["MG94xREV with separate rates for branch sets"]["AIC-c"]; //loglik of MG94XREV
		$this->arrRet["NULL_AICc"] = $arrJSONret["fits"]["RELAX null"]["AIC-c"]; //Loglik of null model assuming K=1 (no relax or intensification of selection)
		$this->arrRet["Alternative_AICc"] = $arrJSONret["fits"]["RELAX alternative"]["AIC-c"]; //loglik of alt model, k estimated.                      
		$this->arrRet["qOne_AICc"] = $arrJSONret["fits"]["RELAX qOne"]["AIC-c"]; //loglik of alt model, k estimated.                      
        ...
        public function RunBUSTEDMH($bKeepTmpFolder=true) {
		...
		$sNexusFile = $this->sTmpFileBase . HyPhyUtil::IN_FILE_NAME;
                ...
                //Run Ctrl file 1: ========================================
                chdir($this->sTmpFileBase ); //go to temp directory
                echo("Running BUSTEDMH...\n");

                $sCMD = HYPHY_PATH . " ".  BUSTED_MH_SCRIPT;//HyPhyUtil::PREFIX_CTRL_FILE1 . ".bf";
		$sCMD .= "  --alignment $sNexusFile --branches T > $sNexusFile.log 2>&1 ";
                echo($sCMD . "\n");
                ...
                $sF1 = fnReadText(  $sNexusFile . ".BUSTED.json"); //read output.
		...
		$arrJSONret = json_decode($sF1 , true);
                if (is_null($arrJSONret) ) {
					...
					$this->arrRet["LRT"] = 0;
					$this->arrRet["P"] = 0;
					$this->arrRet["NULL_LK"] = 0; 
					$this->arrRet["Alternative_LK"] = 0; 
					$this->arrRet["MG94_LK"] = 0;
					$this->arrRet["MG94_double_LK"] = 0;
					$this->arrRet["MG94_triple_LK"] = 0;
		...
		$this->arrRet["LRT"] = $arrJSONret["test results"]["LRT"];
		$this->arrRet["P"] = $arrJSONret["test results"]["p-value"];
		$this->arrRet["NULL_LK"] = $arrJSONret["fits"]["Constrained model"]["Log Likelihood"]; 
		$this->arrRet["Alternative_LK"] = $arrJSONret["fits"]["Unconstrained model"]["Log Likelihood"]; 
                $this->arrRet["MG94_LK"] = $arrJSONret["fits"]["MG94xREV with separate rates for branch sets"]["Log Likelihood"]; 
                $this->arrRet["MG94_double_LK"] = $arrJSONret["fits"]["MG94 with double instantaneous substitutions"]["Log Likelihood"]; 
		$this->arrRet["MG94_triple_LK"] = $arrJSONret["fits"]["MG94 with double and triple instantaneous substitutions"]["Log Likelihood"];
