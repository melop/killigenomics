...
$nParts = 100;
$nMinMinorAlleleFreq = 0.05;
//$nMinMinorAlleleFreq = 0.05;// if the minor allele freq is <= 0.05
$nMinMajorAlleleFreq = 1 - $nMinMinorAlleleFreq; // will be treated as fixed
$sOutgroupFasta = "/beegfs/group_dv/home/RCui/killifish_genomes/denovo/discovardenovo/NOR/LG/v1.2/FTH.chr.fa";
...
while ($nEOFCount < $nParts ) {
	...
	if ( (array_key_exists($sOutgroupBase , $arrBaseFreq)) && ( count($arrBaseFreq) == 1 || $arrBaseFreq[$sOutgroupBase] > $nMinMajorAlleleFreq ) ) { //fixed ancestral sites.
	...
	} else if ( ((!array_key_exists($sOutgroupBase , $arrBaseFreq)) && count($arrBaseFreq) == 1) || ( array_key_exists($sOutgroupBase , $arrBaseFreq) && $arrBaseFreq[$sOutgroupBase] < $nMinMinorAlleleFreq ) ) { //divergent site with the outgroup species.
		/*$arrOutput['ancestralbase'] = $sOutgroupBase;
		...
		*/
		...
		if ( array_key_exists($sOutgroupBase , $arrBaseFreq) && $arrBaseFreq[$sOutgroupBase] < $nMinMinorAlleleFreq ) {
			//Could be divergent, but not sure, exclude.
			$nCurrPart++;
			continue;
		}
