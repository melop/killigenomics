...
*/
$sDIR = $argv[1];//"ORTDRY";
$nHWCriticalValue = 3.841; // critical chisquare value for p = 0.05 
$nPolyCriticalValue = 3.841; // critical chisquare value for p = 0.05
$nMinPopCoverage = 100;
$nMaxPopCoverage = floatval($argv[2]);// 2.229171 * 60 * 2;
$nMinEffectiveChromosomes = 50; //min cov 100, min chr 50, average cov at least 2x
$nMinEffectiveIndividuals = 10;
$nParts = 100;
$nMinMinorAlleleFreq = 0.01;
$nMaxMinorAlleleFreq = 0.98; //only look at polymorphisms segregating at 1% to 98%.

$nMinMajorAlleleFreq = 0.98; // will be treated as fixed
...
while ($nEOFCount < $nParts ) {
	...
	$hToWrite = $hOut;
	if ( (array_key_exists($sOutgroupBase , $arrBaseFreq)) && ( count($arrBaseFreq) == 1 || (1-$arrBaseFreq[$sOutgroupBase]) < $nMinMinorAlleleFreq ) ) { //fixed ancestral sites.
		/*
