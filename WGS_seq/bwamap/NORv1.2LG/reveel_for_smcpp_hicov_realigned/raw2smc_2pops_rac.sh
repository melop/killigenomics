...
OUTDIR=smcpp_2pops_input_rawvcf
sInVCFDIR=`realpath vcfs_2pops`
sMaskDIR=`realpath masks_2pops`
sSp=RAC
arrPops=( ${sSp}WET ${sSp}DRY )
arrDistinguishedInd=( RACWET_96_NR-96-1_S507_D708 RACDRY_43_NR-43-A5_S508_N705 )
arrChr=( `seq 1 19` )
...
OUTDIR=`realpath $OUTDIR`;
   ...
   for nChr in "${arrChr[@]}";do
	...
	WDChr=$WD/$sChr
	mkdir -p $WDChr;
	$SMCPP vcf2smc -m $sMaskDIR/${sSp}.mask.varqual.bed.gz -d "$sDist1" "$sDist1" "$sInVCFDIR/${sSp}.var.vcf.gz" "$WDChr/smc.12.gz" "$sChr" $sIndList1 $sIndList2 > $WDChr/log.12.txt 2>&1 &

	$SMCPP vcf2smc -m $sMaskDIR/${sSp}.mask.varqual.bed.gz -d "$sDist2" "$sDist2" "$sInVCFDIR/${sSp}.var.vcf.gz" "$WDChr/smc.21.gz" "$sChr" $sIndList2 $sIndList1 > $WDChr/log.21.txt 2>&1 &
