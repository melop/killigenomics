	...
	public function LoadGFF($sGFFFile, $sGenome, $sTreatTypeAsCDS = "CDS") { //treat this type as "cds".
		...
		while (($sLn = fgets($hFile)) !== false) {
			...
			if ($sFeatureType == 'mRNA') { // check if gene is there
				...
				$this->arrGenes[$sGenePredictor][$arrAnnot['Parent']]['mRNAs'][$arrAnnot['ID']] = array('scf'=>$arrF[0] , 
												'start' =>$arrF[3],
												 ...
												 'strand' => $arrF[6], 
												'annot' => $arrAnnot,
												'CDSs' => array(), 'CDSsorted' => false, 'CDSLen' => 0);
			...
			if ($sFeatureType == $sTreatTypeAsCDS) { // check if gene is there
				...
				$this->arrGenes[$sGenePredictor][$sGeneID]['mRNAs'][$arrAnnot['Parent']]['CDSLen'] += abs($arrF[4] - $arrF[3]) + 1;
				continue;
			}
...
function fnTranslatorX($arrSeqs , $nPMinSeqConsv=0.75, $nPMinSeqFlank = 0.85, $nMinContNonConsv=2, $nMinBlockLen=5, $nExcludeBelow=0.1) { //if lower than 10% left after trimming, exclude
	...
	foreach($arrSeqs as $sName => $sSeq) {
		$arrNameTable[$nSeqCount] = $sName;
		fwrite($hIn, ">".$nSeqCount."\n".$sSeq."\n");
		$nSeqCount++;
	}
	...
	$sAln = fnExcludeShortSeqFas($sTmp."/tranX.nt_ali.fasta" , 'dna', $arrNameTable, -1);
	$sCleanedAln = fnExcludeShortSeqFas($sTmp."/tranX.nt_cleanali.fasta" , 'dna', $arrNameTable, $nExcludeBelow);
	$sAlnAA = fnExcludeShortSeqFas($sTmp."/tranX.aa_ali.fasta", 'aa', $arrNameTable, -1);
	$sCleanedAA = fnExcludeShortSeqFas($sTmp."/tranX.aa_cleanali.fasta" , 'aa' , $arrNameTable, $nExcludeBelow);

	exec("rm -R $sTmp");
	return(array($sAln , $sCleanedAln, $sAlnAA, $sCleanedAA ));
}

function fnExcludeShortSeqFas($sFas , $nMode='dna', $arrNameTable , $nPercentCutoff) {
	$hFas = fopen($sFas , 'r');
	$sSeqName = "";
	$sSeq = "";
	...
	do {
		...
		if ($sLn==false) {
			if ($sSeq != '') {
				$sRet .= fnExcludeShortSeqFasProc($sSeqName, $sSeq, $nMode, $nPercentCutoff);
			}
			break;
		}
		...
		if (substr($sLn , 0,1) == ">") {
			...
			if (array_key_exists($sSeqName , $arrNameTable)) {
				$sSeqName  = $arrNameTable[$sSeqName];
			}
			$sSeq = "";
...
function fnExcludeShortSeqFasProc($sSeqName, $sSeq, $nMode='dna', $nPercentCutoff) {
	$nOrigLen = strlen($sSeq);
	$nTrimLen = ($nMode=='dna')? strlen(str_replace(array('-', 'N' , 'n') , '', $sSeq )) : strlen(str_replace(array('-', 'X' , 'x') , '', $sSeq ));

	if ($nTrimLen/$nOrigLen >= $nPercentCutoff ) {
		return ">$sSeqName\n".wordwrap( $sSeq , 75 ,  "\n" , true )."\n";
	} else {
		return "";
	}
...
function fnIsNonSyn($sCodon, $bMutatedBase) {
	global $CodonNonSyn1stBaseChange, $CodonNonSyn2ndBaseChange, $CodonNonSyn3rdBaseChange;
	$sCodon = strtoupper($sCodon);
	$Table = null;
	switch ($bMutatedBase) {
		case 0: //position 1
		 $Table = $CodonNonSyn1stBaseChange;
		break;
		case 1: //position 2
		return true;
		break;
		case 2: //position 3
		$Table = $CodonNonSyn3rdBaseChange;
		break;
		default:
		die("fnIsNonSyn, mutated bases can only be 0,1,2");
	}

	if (!array_key_exists($sCodon , $Table)) {
		return -1;
	}
