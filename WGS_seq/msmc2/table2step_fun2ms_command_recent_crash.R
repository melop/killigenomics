#setwd("/beegfs/group_dv/home/RCui/killifish_genomes/WGS_seq/bwamap/NORv1.2LG/msmc2/");
...
#This script simulates the sampling being in captivity for n generations at a low effective pop size, like 10


nRecentBottleNeckGen <- 100; #bottlenecked for 100 generations (since domestication)
nRecentBottleNeckSize <- 50; # effective pop size 10
nRecoverGen <- 99;
nRecoverSize <- 1e3;
...
datMSMC1[datMSMC1$gen==0, 'popsize'] <- nRecoverSize;
datMSMC1 <- rbind( datMSMC1[0,], datMSMC1[0,] , datMSMC1);
datMSMC1[2, 'gen'] <- nRecoverGen;
datMSMC1[2, 'popsize'] <- nRecoverSize;
datMSMC1[3, 'gen'] <- nRecentBottleNeckGen;
datMSMC1[3, 'popsize'] <- nRecentBottleNeckSize;
...
datMSMC2[datMSMC2$gen==0, 'popsize'] <- nRecoverSize;
datMSMC2 <- rbind( datMSMC2[0,], datMSMC2[0,] , datMSMC2);
datMSMC2[2, 'gen'] <- nRecoverGen;
datMSMC2[2, 'popsize'] <- nRecoverSize;
datMSMC2[3, 'gen'] <- nRecentBottleNeckGen;
datMSMC2[3, 'popsize'] <- nRecentBottleNeckSize;
