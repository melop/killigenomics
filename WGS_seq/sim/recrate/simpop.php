...
die();
*/
$sMacs = "/software/source/macs/macs"; //I changed the version to print more digits in the positions to avoid collisions. or specify -p digits in the original version.
$sMSFormatter = "/software/source/macs/msformatter"; //I changed the version to print more digits in the positions to avoid collisions. or specify -p digits in the original version.
...
$hOutRaw = fopen($sOutFile.".raw.txt" , 'w');

echo("Calling macs (pop1 $sPop1Name $nPop1Chr pop2 $sPop2Name $nPop2Chr locus length $nLocusLen)...\n");
$sCmd = "$sMacs $nTotalChr $nLocusLen -t $nPersiteTheta -h 1e2 -r $nPersiteRho -R $sRecFile -I 2 $nPop1Chr $nPop2Chr $sDemography 2>/dev/null | $sMSFormatter ";
echo("$sCmd\n");
$hIn = popen( $sCmd , 'r' );
...
while( false !== ($sLn = fgets($hIn) ) ) {
	...
	if ( $sLn == '//') {
		$bResultReached = true;
		echo("macs responded, waiting for output...\n");
		continue;
	}
