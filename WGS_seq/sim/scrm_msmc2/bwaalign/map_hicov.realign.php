...
$sFQDIR = "../hicovreads_err0.0033/";
$sOutDIR = "./mapped_hicov_gatk_realigned/";
...
//$sRefGenome = "ref.fa";
$sRefGenome = "chr1-19.fa";
$nMinMapQ = 30; // skip low quality alignments; only used for calling variants.
...
//echo("$sFQDIR/*_R1.fq.gz\n");
$arrR1 = glob("$sFQDIR/*_read1.fastq.gz");
//print_r($arrR1);
...
for($i=0;$i<count($arrR1);$i++) {
	...
	$sR1 = $arrR1[$i]; //realpath($arrR1[$i]);
	$sFQDIR = $sFQDIR; //realpath($sFQDIR);

	$sR1FileName = basename($sR1);
	$sR2FileName = str_replace("_read1.fastq.gz" , "_read2.fastq.gz" , $sR1FileName);
	$sR2 = $sFQDIR."/$sR2FileName";

	$sFileStem = str_replace("_read1.fastq.gz"  , '', $sR1FileName);
	...
	$sCmd = "[[ ( ! -e $bMapDoneFlag ) && ( ! -L $bMapDoneFlag )  ]] && $sBWA mem -t $nPerJobThread \"$sRefGenome\" \"$sR1\" \"$sR2\" | $SAMTOOLS view -Su - | $SAMTOOLS sort - $sSpSortedBamPrefix && touch $bMapDoneFlag;";
	echo($sCmd."\n");
	exec($sCmd);
	...
	//indel realignment
	$sCmd = "if [[ (  -e $bRealignedBamDoneFlag ) || ( -L $bRealignedBamDoneFlag )  ]]; then echo $bRealignedBamDoneFlag was finished. skip; else ";
