<?php
//this script reads in the Ensembl peptide file, selects the longest isoform for a gene, replace the "." in the ensembl ID with underscore _, and remove all content after the first space. a tab file is output to stderr.
...
function fnCheck($sSeqName , $sSeq) {
	...
	preg_match("/gene:(\S+)/", $sSeqName , $arrM);

	if (count($arrM) !=2 ) return;
