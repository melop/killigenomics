...
class MappedCDSGFF { //parse the CDS annotations in GFF and provide easy ways to access positional information
	...
	public function ExtractmRNASequence($sGenePredictor, $smRNAID) { // 
		if (!array_key_exists($smRNAID , $this->arrRNA2Gene) ) {
			die("mRNA ID not found: $smRNAID\n");
		}
		$oRNA = ($this->arrGenes[$sGenePredictor][$this->arrRNA2Gene[$smRNAID]]['mRNAs'][$smRNAID]);

		return $this->ExtractmRNASequenceByModel($oRNA);
	}

	public function ExtractmRNASequenceByModel(&$oRNA) { // 
	...
	public static function fnParseAnnotation($s) {
		$arrMap = array();
		$arrF = explode(';' , $s);
		foreach($arrF as $v) {
			$arrPair = explode('=' , $v);
...
function fnTranslatorX($arrSeqs , $nPMinSeqConsv=0.5, $nPMinSeqFlank = 0.85, $nMinContNonConsv=4, $nMinBlockLen=5) {
