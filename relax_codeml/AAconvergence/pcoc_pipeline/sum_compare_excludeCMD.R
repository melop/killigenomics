...
sCodeMLRet <- "../codeml_46_spp/rerun_exclude_MNM/";
sRelaxRet <- "../relax_46_spp_exclude_MNM/rerun_omega0";
...
datPcoc <- datPcoc[ ! (paste(datPcoc$OrthoID, datPcoc$AASite, sep=":") %in% datCMDList$Pos),  ]; #remove CMD sites!
...
write.table(datPcoc[datPcoc$PCOC>=nMinPosteriorProb,], paste(sDir,"/pcoc.sig.excludeCMD.txt",sep=""), col.names = T, row.names = F, quote=F, sep="\t" );

sink(file = paste(sDir,"/fisher.test.excludeCMD.txt",sep="") );
