...
class AUTest {
	...
	public function SetModel($sM) {
		$this->sModel = $sM;
	}

	public function DoTest($bOnlyReturnSiteLK = false) {
		...
		do {
			...
			mkdir($this->sTempDir);
			chdir($this->sTempDir);
			echo("temp dir: ".realpath($this->sTempDir)."\n");
			break;
		} while(true);
		...
		$sCmd = RAXML_PATH . " -f G -s " . AUTest::IN_FILE_NAME . " -m ". $this->sModel." -z ". AUTest::TREE_FILE_NAME ." -n ".AUTest::RAXML_OUT; //changed -f g to -f G, to re-estimate params for the forced tree.
		echo($sCmd."\n");
		exec( $sCmd );
		...
		//now do consel
		if ($bOnlyReturnSiteLK) {

			$sRet = file_get_contents('RAxML_perSiteLLs.output');
			chdir(dirname(__FILE__) );
