	...
	do {
	  ...
	  chdir($this->sCurrDir);
		...
		$sNexus .= "END;\n";
                ...
                $sCMD = "export OMP_NUM_THREADS=4; " . HYPHY_PATH . "/bin/HYPHYMP CPU=4 ".  HyPhyUtil::PREFIX_CTRL_FILE1 . ".bf"; //CPU=2 needed to introduce randomness.

                
                //return;
		echo("start time: ".exec("date") . " php time:".time()."\n");
                echo($sCMD . "\n");
                $sRet = exec("$sCMD ");
		...
		$this->arrRet["K"] = $arrJSONret["test results"]["relaxation or intensification parameter"]; //estimated k
		$this->arrRet["LR"] = $arrJSONret["test results"]["LRT"]; //likelihood ratio
		$this->arrRet["P"] = $arrJSONret["test results"]["p-value"]; //p value with 1 degree of freedom. chi sq.
