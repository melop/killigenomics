...
//$sMonoDesc = "monophyly_force_callo_nothos_monophyly.txt";
//$sMonoDesc = "monophyly_force_callo_nothos_monophyly2.txt";
$sMonoDesc = "monophyly_force_callo_nothos_monophyly3.txt";
//$sPremadeFasta = "ret_cleanDNA.fasta"; //ret_improved_cleanDNA.fasta
//$sOutDIR = "genetrees";

$sPremadeFasta = "ret_improved_cleanDNA.fasta";
//$sOutDIR = "genetrees_forced_callo_nothos_monophyly";
//$sOutDIR = "genetrees_forced_callo_nothos_monophyly2";
$sOutDIR = "genetrees_forced_callo_nothos_monophyly3";
...
function fnProcessAln($sGeneName,$arrFullCodingSeq) {
	...
	if (array_key_exists($sGeneName , $arrComputedResults) && $arrComputedResults[$sGeneName][2] != 'accepted_monophyly'  ) {
		...
		if (!$arrAlnRet["stopcodon"] ) { //there is no stop codon, do raxml
			...
			echo("AU RET:\n");
			print_r($oAURet);
			$arrAURet = explode("\t", trim($oAURet[1]));
			$nLnLkDiff = abs(floatval($arrAURet[0]));
			$nAUPvalue = floatval($arrAURet[1]);
			echo("AU debug: ".$arrAURet[0]."; $nLnLkDiff ; ".$arrAURet[1]."; $nAUPvalue\n");
			$sConstraintAccepted = ($nAUPvalue > 0.05)? "accepted_monophyly" : "rejected_monophyly";
